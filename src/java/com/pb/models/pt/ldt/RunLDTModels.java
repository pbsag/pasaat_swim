...
//        String skimsPath = ResourceUtil.getProperty(appRb, "assign.curent.skim.path");
//        String[] fileNames = ResourceUtil.getArray(appRb, "pt.car.peak.skims");
//        logger.info("Reading times in " + skimsPath + fileNames[0]);
//        Matrix time = MatrixReader.readMatrix(new File(skimsPath + fileNames[0]), "time");
//        logger.info("Reading distances in " + skimsPath + fileNames[1]);
//        Matrix dist = MatrixReader.readMatrix(new File(skimsPath + fileNames[1]), "dist");
      
        String hwyPath = ResourceUtil.getProperty(appRb, "highway.assign.previous.skim.path");
        String[] fileNames = ResourceUtil.getArray(appRb, "pt.car.peak.skims");
        logger.info("Reading time in " + hwyPath + fileNames[0]+".zmx");
        Matrix time = readTravelCost(hwyPath + fileNames[0]+".zmx", "carPkTime");
        logger.info("Reading distance in " + hwyPath + fileNames[1]+".zmx");
        Matrix dist = readTravelCost(hwyPath + fileNames[1]+".zmx", "carPkDist");
