...
public class Auto extends Mode {
    ...
    protected static Logger logger = Logger.getLogger(Auto.class);
    
    private final static long serialVersionUID = 288;

    private static int m; 
    
    private float rentalCarCost; 
    private float[] averageNumDays;
    
    public Auto(ResourceBundle rb) {
        
        isAvailable = true;
        hasUtility = false;
        utility = 0.0D;
        alternativeName = new String("Auto");
        type = LDTourModeType.AUTO; 
        m = type.ordinal(); 
     
        // get some parameters used to calculate cost
//        rentalCarCost  = (float) ResourceUtil.getDoubleProperty(rb, "ldt.rental.car.cost.cents.per.day");
        PriceConverter priceConverter = PriceConverter.getInstance(); //will throw exception if not initialized yet
        rentalCarCost = (float) priceConverter.convertPrice(ResourceUtil.getDoubleProperty(rb, "ldt.rental.car.cost.cents.per.day"),PriceConverter.ConversionType.PRICE);
    ...
    public boolean calculateUtility(float[] c, LDModeChoiceHouseholdAttributes hh,
            LDTour tour, LDTravelTimeAndCost tc, boolean otherModesAvailable) {
        ...
        // set availability
        isAvailable = true;         
        if (otherModesAvailable) {
            if (tc.inVehicleTime[m]==0) isAvailable = false;
            if (completeTour==1) {
                if (tc.inVehicleTime[m] > (tour.schedule.duration)) {
                    isAvailable = false; 
                }
            }            
        }     
        
        // if no HH vehicle, must pay to rent a car
        float cost = tc.cost[m];
        if (hh.autos==0) {
            if (tour.patternType==LDTourPatternType.COMPLETE_TOUR)
                cost += rentalCarCost;
            else     
                cost += rentalCarCost * averageNumDays[tour.purpose.ordinal()];
        }
        ...
        if (isAvailable) {
            ...
            utility = 0;            
            utility += c[INVEHICLETIME    ] * tc.inVehicleTime[m];      
