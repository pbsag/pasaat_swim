...
public class LDSkimsInMemory {
    ...
    public void readSkimsIntoMemory (ResourceBundle globalRb, ResourceBundle rb) {
        ...
        String readSkimsString = ResourceUtil.getProperty(rb, "ldt.read.offpeak.skims.only");
    ...
    private void readSkims(ResourceBundle rb) {
        ...
        for (int m=0; m<mode.length; m++) {
            String groupName  = "pt." + mode[m].getGroupName() + ".offpeak";
            String[] tableLabels = mode[m].getTableLabels();
            opSkimTables[m] = createHashSet(tableLabels);
            opSkims[m] = readMatrixCollection(rb,mode[m] == LDTourModeType.AUTO ? highwayPath : transitPath, groupName, tableLabels);
        }

        // flag used in testing to save memory
        for (int m=0; m<mode.length; m++) {
                String groupName  = "pt." + mode[m].getGroupName() + ".peak";
                String[] tableLabels = mode[m].getTableLabels();
                pkSkimTables[m] = createHashSet(tableLabels);
                pkSkims[m] = readMatrixCollection(rb,mode[m] == LDTourModeType.AUTO ? highwayPath : transitPath, groupName, tableLabels);
        }
    ...
    private MatrixCollection readMatrixCollection(ResourceBundle rb,
            String path, String groupName, String[] tableLabel) {
        long startTime = System.currentTimeMillis();
        MatrixCollection mc;

        // get the actual file names
        String[] fileNames = ResourceUtil.getArray(rb, groupName + ".skims");
        if (fileNames.length != tableLabel.length) {
            
            String expectedTables = ""; 
            for (int i=0; i<tableLabel.length; i++) {
                expectedTables += "  " + tableLabel[i];
            }            
            
            logger.error("Error attempting to read long distance matrix collection.");
            logger.error("  Wrong number of matrices specified in property file for group: " + groupName);            
            logger.error("  Expected tables: " + expectedTables);
            
            throw new RuntimeException("Wrong number of tables in properties file");
        }

        // read the index matrix and create the collection
        Matrix index = readMatrix(path, fileNames[0], groupName, tableLabel[0]);
        ...
        for (int i = 1; i < tableLabel.length; ++i) {
            Matrix m = readMatrix(path, fileNames[i], groupName, tableLabel[i]);
            mc.addMatrix(m);
        }
     ...
     * @param groupName Name of the matrix group ("carOp", "carPk", "icdtPk", etc)
     * @param tableName Name of the specific tables ("ivt", "fwt", "xwt", etc)
     * @return A matrix read from disk.  
     */
    private Matrix readMatrix(String path, String fileName, String groupName, String tableName) {

        Matrix m = null;
        try {
            logger.info("Reading travel costs in " + path + fileName);
            File file = new File(path+fileName + MATRIX_EXTENSION);
            m = MatrixReader.readMatrix(file, groupName + tableName);
            m.setName(tableName);
        } catch (Exception e) {
